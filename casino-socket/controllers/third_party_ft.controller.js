...
exports.getFT_FS = async (thirdPartyAuthData) => {
	try {
...
exports.getFT_FU_R_TODAY = async (thirdPartyAuthData) => {
	try {
		let thirdPartyBaseUrl = thirdPartyAuthData["thirdPartyBaseUrl"];
		let thirdPartyUrl = "";
		let version = thirdPartyAuthData["version"];
...
exports.dispatchFT_DEFAULT_TODAY = (itemList) => {
	itemList.map(async item => {	
		try {
			response = await axios.post(`${BACKEND_BASE_URL}${MATCH_SPORTS}${SAVE_FT_FU_R}`, item);
			console.log("response==============================", response.data);
		} catch(err) {
			console.log("err===================", err);
		}
