...
class PublicUtils
{
	...
	public static function MoneyToSsc($t_user){
    ...
    public static function show_voucher($line, $id)
    {  //生成注单号
        switch ($line) {
            case 1:
    ...
    //大小球计算：
    public static function odds_dime($mbin1, $tgin1, $dime, $mtype)
    {
    ...
    //让球计算:
    public static function odds_letb($mbin, $tgin, $showtype, $dime, $mtype)
    {
    ...
    //上半大小球计算：
    public static function odds_dime_v($mbin1, $tgin1, $dime, $mtype)
    {
        if ($mbin1 < 0 or $tgin1 < 0) return 0;
        $dime = str_replace('大', '', $dime);
        $dime = str_replace('小', '', $dime);
    ...
    //上半让球计算:
    public static function odds_letb_v($mbin, $tgin, $showtype, $dime, $mtype)
    {
        if ($mbin < 0 or $tgin < 0) return 0;
        if (eregi("[+]", $dime)) {
            $letb_odds = explode("+", $dime);
    ...
    //滚球大小球计算：
    public static function odds_dime_rb($mbin1, $tgin1, $dime, $mtype)
    {
        if ($mbin1 < 0 or $tgin1 < 0) return 0;
        $dime = str_replace('大', '', $dime);
        $dime = str_replace('小', '', $dime);
    ...
    //滚球让球计算:
    public static function odds_letb_rb($mbin, $tgin, $showtype, $dime, $mtype)
    {
        if ($mbin < 0 or $tgin < 0) return 0;
        if (eregi("[+]", $dime)) {
            $letb_odds = explode("+", $dime);
    ...
    //滚球上半大小球计算：
    public static function odds_dime_vrb($mbin1, $tgin1, $dime, $mtype)
    {
        if ($mbin1 < 0 or $tgin1 < 0) return 0;
        $dime = str_replace('大', '', $dime);
        $dime = str_replace('小', '', $dime);
    ...
    //滚球上半让球计算:
    public static function odds_letb_vrb($mbin, $tgin, $showtype, $dime, $mtype)
    {
        if ($mbin < 0 or $tgin < 0) return 0;
        if (eregi("[+]", $dime)) {
            $letb_odds = explode("+", $dime);
    ...
    //波胆计算：
    public static function odds_pd($mb_in_score, $tg_in_score, $m_place)
    {
    ...
    //上半波胆计算：
    public static function odds_pd_v($mb_in_score_v, $tg_in_score_v, $m_place)
    {
    ...
    //单双计算:
    public static function odds_eo($mb_in_score, $tg_in_score, $m_place)
    {
        if ($mb_in_score < 0 or $tg_in_score < 0) return 0;
    ...
    //入球数计算:
    public static function odds_t($mb_in_score, $tg_in_score, $m_place)
    {
        if ($mb_in_score < 0 or $tg_in_score < 0) return 0;
        $inball = $mb_in_score + $tg_in_score;
    ...
    //入球数计算:
    public static function odds_bst($mb_in_score, $tg_in_score, $m_place)
    {
        if ($mb_in_score < 0 or $tg_in_score < 0) return 0;
        $inball = $mb_in_score + $tg_in_score;
    ...
    //独赢计算：
    public static function win_chk($mbin, $tgin, $m_type)
    {
        if ($mbin < 0 or $tgin < 0) return 0;
        $grade = 0;
        switch ($m_type) {
    ...
    //上半独赢计算：
    public static function win_chk_v($mbin, $tgin, $m_type)
    {
        if ($mbin < 0 or $tgin < 0) return 0;
        $grade = 0;
        switch ($m_type) {
    ...
    //独赢计算：
    public static function win_chk_rb($mbin, $tgin, $m_type)
    {
        if ($mbin < 0 or $tgin < 0) return 0;
        $grade = 0;
        switch ($m_type) {
    ...
    //上半独赢计算：
    public static function win_chk_vrb($mbin, $tgin, $m_type)
    {
        if ($mbin < 0 or $tgin < 0) return 0;
        $grade = 0;
        switch ($m_type) {
    ...
    //标准过关计算：
    public static function odds_p($mid, $mtype, $mrate)
    {
        $winrate = 1;
        $mid = explode(',', $mid);
        $mtype = explode(',', $mtype);
    ...
    //让球过关计算：
    public static function odd_pr($mid, $mtype, $mrate, $mplace, $showtype)
    {
        $winrate = 1;
        $mid = explode(',', $mid);
        $mtype = explode(',', $mtype);
