...
class KYUtils
{
    ...
    function getKYGameRecords($sc)
    {
        ...
        if ($jsonStr != "") {
            ...
            $tmpfile = storage_path('app/public/tmp/ky_') . date("Ymd") . ".txt";
            $f = fopen($tmpfile, 'a');
    ...
    function KY_Deposit($username, $Gold, $tp = "IN")
    { //成功返回1 失败返回0
        $aes = new DES();
        $lineCode = $_SERVER['HTTP_HOST'];
        $timestamp = str_pad($aes->getMillisecond(), 13, 0); //时间戳
    ...
    function KY_Money2($username, $type = 2)
    { //成功返回数组，失败返回0
        $aes = new DES();
        $lineCode = $_SERVER['HTTP_HOST'];
        $timestamp = str_pad($aes->getMillisecond(), 13, 0); //时间戳
    ...
    function Get_Orderid_Status($orderid, $type = 1)
    {
        ...
        $jsonStr = $this->getUrl_KY($url);
        $jsonData = json_decode($jsonStr, true);
        if ($jsonData['d']['status'] == 0 and count($jsonData, 1) > 1) {
            $jsonData['d']['status'] = 1;
        }
        //成功改为1
        if (strlen($jsonStr) < 5) {
