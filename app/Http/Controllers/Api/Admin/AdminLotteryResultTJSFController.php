...
class AdminLotteryResultTJSFController extends Controller
{
    public function getLotteryResult(Request $request)
    {
        ...
        try {
            ...
            $lottery_type = "天津十分彩";
            $result = LotteryResultTJSF::whereDate("datetime", "=", $query_time);
            if ($qishu_query != "") {
            ...
            $response['message'] = "Lottery Result TJSF Data fetched successfully!";
    ...
    public function getLotteryResultById(Request $request)
    {
        ...
        try {
            ...
            $id = $request_data["id"];
            $result = LotteryResultTJSF::find($id);
    ...
    public function saveLotteryResult(Request $request)
    {
        ...
        try {
            ...
            if ($action == "add" && $id == 0) {
                ...
                $result = LotteryResultTJSF::where("qishu", $qishu)->first();
                ...
                $item = LotteryResultTJSF::find($id);
            ...
            $response['message'] = "TJSF Lottery Result updated successfully!";
            $response['success'] = TRUE;
            $response['status'] = STATUS_OK;
        } catch (Exception $e) {
            $response['message'] = $e->getMessage() . ' Line No ' . $e->getLine() . ' in File' . $e->getFile();
    ...
    public function checkoutResult(Request $request)
    {
        ...
        try {
            ...
            $g_type = $request_data["g_type"] ?? "TJSF";
            $qishu = $request_data["qishu"];
            ...
            foreach ($orders as $order) {
                ...
                if (in_array($selectBall, array("1", "2", "3", "4", "5", "6", "7", "8"))) {
                    ...
                    if (in_array($betContent, array(
                        $result['ball_1'], $result['ball_2'], $result['ball_3'], $result['ball_4'], $result['ball_5'],
