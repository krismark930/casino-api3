...
class AdminLotteryResultXYFTController extends Controller
{
    public function getLotteryResult(Request $request)
    {
        ...
        try {
            ...
            $lottery_type = "幸运飞艇";
            $result = LotteryResultXYFT::whereDate("datetime", "=", $query_time);
            if ($qishu_query != "") {
            ...
            $response['message'] = "Lottery Result XYFT Data fetched successfully!";
    ...
    public function getLotteryResultById(Request $request)
    {
        ...
        try {
            ...
            $id = $request_data["id"];
            $result = LotteryResultXYFT::find($id);
    ...
    public function saveLotteryResult(Request $request)
    {
        ...
        try {
            ...
            if ($action == "add" && $id == 0) {
                ...
                $result = LotteryResultXYFT::where("qishu", $qishu)->first();
                ...
                $item = LotteryResultXYFT::find($id);
            ...
            $response['message'] = "XYFT Lottery Result updated successfully!";
            $response['success'] = TRUE;
            $response['status'] = STATUS_OK;
        } catch (Exception $e) {
            $response['message'] = $e->getMessage() . ' Line No ' . $e->getLine() . ' in File' . $e->getFile();
    ...
    public function checkoutResult(Request $request)
    {
        ...
        try {
            ...
            $g_type = $request_data["g_type"] ?? "XYFT";
            $qishu = $request_data["qishu"];
