...
class AdminSysconfigController extends Controller
{
    ...
    public function updateUSDTConfig(Request $request) {
        ...
        try {   
            ...
            $USDT = $request_data["USDT"];
            $tjck = $request_data["tjck"];
            $ckfanli = $request_data["ckfanli"];

            $sys_config = SysConfig::query()->update([
                "USDT" => $USDT,
                "tjck" => $tjck,
                "ckfanli" => $ckfanli,
            ]);

            $response['message'] = "USDT Config Data updated successfully!";
            $response['success'] = TRUE;
            $response['status'] = STATUS_OK;
        } catch (Exception $e) {
            $response['message'] = $e->getMessage() . ' Line No ' . $e->getLine() . ' in File' . $e->getFile();
