...
class BBINController extends Controller
{
    ...
    public function getBBINTransaction(Request $request) {
        ...
        try {
            ...
            $sysConfig = SysConfig::all()->first();
            $agentCode = 'H07'.$sysConfig['AG_User'];
            $key = '';

            $url="http://999.bbin-api8.com/bbin_data.php?agentCod=$agentCode&key=";

            $web_report_zr = WebReportZr::where("platformType", 'BBIN')
                ->where("Checked", 1)
