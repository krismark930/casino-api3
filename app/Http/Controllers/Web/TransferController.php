...
class TransferController extends Controller {
    ...
    function handleMG(Request $request, User $user, SysConfig $sysConfig ) {
        ...
        if($MG_username==null or $MG_username==""){
            ...
            if($result['info']=='0'){
                $update = User::where('UserName', $username)->update(['MG_User' => $ag_username,
                'MG_Pass' => $MG_password]);
                if($update){
        ...
        if($tp=="OUT"){  //转出
            $money2= AGUtils::getMoney($MG_username, $MG_password); //获取真人余额
            if($money>$money2){
                return response()->json(['success'=>false, 'message'=> '转账金额不能大于真人账号余额!']);
            }
        }
    ...
    function handlePT(Request $request, User $user, SysConfig $sysConfig ) {
        ...
        if($PT_username==null or $PT_username==""){
            $WebCode =ltrim(trim($sysConfig['AG_User']));
            if(!preg_match("/^[A-Za-z0-9]{4,12}$/", $user['UserName'])){
                $PT_username = PTUtils::getpassword_MG(10);
            }else{
                $PT_username=trim($user['UserName']).PTUtils::getpassword_MG(1);
            }
            $PT_username='h07'.$WebCode.$PT_username;
            $PT_username=strtolower($PT_username);
            $PT_password=strtolower(PTUtils::getpassword_MG(10));
            $result= PTUtils::Addmember_MG($PT_username,$PT_password,1);
            if($result['info']=='0'){
